...
char analyse_where_to_go_1(int distance[SONAR_NUM]) {
  ...
  if (end_wall(distance[LEFT]) == false && end_wall(distance[STRAIGHT]) == false && end_wall(distance[RIGHT]) == false) {
    return 'P'; // end of maze when no walls = pause
  } else if (close_wall(distance[LEFT]) == true || close_wall(distance[STRAIGHT]) == true || close_wall(distance[RIGHT]) == true) {
    ...
    return 'N'; // left
  } else if (wall(distance[STRAIGHT]) == false && wall(distance[RIGHT]) == false) {
    return 'S'; // straight, wenn no wall on right side
  } else if (wall(distance[STRAIGHT]) == false && wall(distance[RIGHT]) == true) {
